        ...
        <ErrorMessage message="Data not found"/>) : (
            ...
            <article className='flex-1'>
               ...
               <label htmlFor='postPicture' className='w-full cursor-pointer'>
                {
                    photo ? (
                        <img src={URL.createObjectURL(photo)} alt={data?.title}
                        className='rounded-xl w-full' />
                    ) : initialPhoto ? (
                        <img src={stables.UPLOAD_FOLDER_BASE_URL + data?.photo} alt={data?.title}
                        className='rounded-xl w-full' />
                    ) : (
               ...
               <input 
                ...
                <div className='mt-4 flex gap-2'>
                 {data?.categories.map((category)=>(
                    <Link to={`/blog?catgory=${category.name}`} 
                    className='text-primary text-sm font-roboto font-bold inline-black md:text-base'>{category.name}</Link>
                  ))}
                </div>
                
                <h1 className='text-xl font-medium font-roboto mt-4 text-dark-hard md:text-[26px]'>{data?.title}</h1>
                <div className='w-full'>
                    {!isLoading && !isError && ( <Editor content={data?.body} editable={true} onDataChange={(data)=>{
                        setBody(data);
                    }}/>)}
