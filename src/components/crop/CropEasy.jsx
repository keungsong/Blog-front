       ...
       <div className='bg-white h-fit w-full sm:max-w-[350px] p-5 rounded-lg'>
         <h2 className='font-semibold text-dark-hard mb-2'>Crop image</h2>
          ...
          <div >
            ...
            className='block mt-2 mb-0.5 text-sm font-medium text-gray-900'> Zoom:{`${Math.round(zoom * 100)}%`}</label>
            <input 
            type="range" 
            id='zoomRange'
            min={1}
          ...
          <div className='flex justify-between gap-2 flex-wrap'>
            ...
            <button disabled={isLoading} onClick={handleCropImage} className='px-5 py-2.5 rounded-lg text-white border bg-blue-500 text-sm disabled: opacity-70'>Crop and Upload</button>
          </div>
       </div>
    </div>
  )
