...
import {BiMessage,BiEditAlt,BiTrashAlt} from 'react-icons/bi'
...
const Comment = ({
  ...
  return (
    <div className='flex flex-nowrap items-start gap-x-3 bg-[#F2F4F5] p-3 rounded-lg'>
       <img src={comment?.user?.avatar ? stables.UPLOAD_FOLDER_BASE_URL + comment.user.avatar : images.UserProfile} alt="user profile" className='w-9 h-9 object-cover rounded-full' />
       <div className='flex flex-1 flex-col'>
          <h5 className='text-bold text-dark-hard text-xs lg:text-sm'>{comment.user.name}</h5>
          ...
          <div className='flex items-center gap-x-3 text-dark-light font-roboto text-sm mt-3 mb-3'>
            {isUserLoggined && (
            <button className='flex items-center space-x-2' onClick={() => setAffectedComment({
              type:"replying", _id: comment._id
              })
            }>
            ...
            <button className='flex items-center space-x-2' onClick={()=>deleteComment(comment._id)}>
              <BiTrashAlt className='w-4 h-auto'/>
              <span>Delete</span>
            </button>
              </>
             )}
